...
void turnMe() {
	...
	while (1)
	{
		display_int(180 + (MAX_STEERING_ANGLE - (lightValue * translationFactor)), i++);
		...
		if (lightValue <= OPTIMAL_LIGHT_VALUE) {
			...
			rotateMotorToAngle(100, 40, 180 + (MAX_STEERING_ANGLE - (lightValue * translationFactor)), NXT_PORT_A, 0);
